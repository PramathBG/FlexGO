...
float GIN_node_mlp_2_weight_float[NUM_LAYERS][EMB_DIM][GIN_MLP_1_OUT];
float GCN_convs_root_emb_weight_GIN_node_mlp_2_bias_float[NUM_LAYERS][EMB_DIM];
float PNA_node_conv_weight_float[PNA_NUM_LAYERS][PNA_EMB_DIM][NUM_SCALERS][NUM_AGGRS][PNA_EMB_DIM];

float GIN_node_mlp_eps_PNA_avg_deg_float[NUM_LAYERS];

float bn_sqrt_var_PNA_graph_mlp_2_bias_float[NUM_LAYERS][EMB_DIM];
float bn_weight_PNA_graph_mlp_1_weight_float[PNA_GRAPH_MLP_1_OUT][EMB_DIM];
float bn_bias_PNA_graph_mlp_1_bias_float[NUM_LAYERS][EMB_DIM];
float bn_mean_PNA_graph_mlp_2_weight_float[PNA_GRAPH_MLP_2_OUT][EMB_DIM];

float graph_pred_PNA_graph_mlp_3_weight_float[NUM_TASK][EMB_DIM];
float graph_pred_PNA_graph_mlp_3_bias_float[NUM_TASK];

float bn_weight_PNA_graph_mlp_1_weight_float_PNA[PNA_GRAPH_MLP_1_OUT][PNA_EMB_DIM];
float bn_mean_PNA_graph_mlp_2_weight_float_PNA[PNA_GRAPH_MLP_2_OUT][PNA_GRAPH_MLP_1_OUT];
...
void load_weights(int GNN_instruction)
{
    ...
    switch(GNN_instruction)
    {
        case 0 : {
        	    ...
        	    fread(bn_weight_PNA_graph_mlp_1_weight_float[0], sizeof(float), 32, f);
        	    ...
        	    fread(bn_mean_PNA_graph_mlp_2_weight_float[0], sizeof(float), 32, f);
        	    ...
        	    fread(bn_weight_PNA_graph_mlp_1_weight_float[1], sizeof(float), 32, f);
        	    ...
        	    fread(bn_mean_PNA_graph_mlp_2_weight_float[1], sizeof(float), 32, f);
        	    ...
        	    fread(bn_weight_PNA_graph_mlp_1_weight_float[2], sizeof(float), 32, f);
        	    ...
        	    fread(bn_mean_PNA_graph_mlp_2_weight_float[2], sizeof(float), 32, f);
        	    ...
        	    fread(bn_weight_PNA_graph_mlp_1_weight_float[3], sizeof(float), 32, f);
        	    ...
        	    fread(bn_mean_PNA_graph_mlp_2_weight_float[3], sizeof(float), 32, f);
        	    ...
        	    fread(bn_weight_PNA_graph_mlp_1_weight_float[4], sizeof(float), 32, f);
        	    ...
        	    fread(bn_mean_PNA_graph_mlp_2_weight_float[4], sizeof(float), 32, f);
        	    ...
        	    fread(graph_pred_PNA_graph_mlp_3_weight_float, sizeof(float), 32, f);
                ...
                for(int l = 0; l < NUM_LAYERS; l++) {
                    ...
                    for(int i = 0; i < EDGE_ATTR; i++) {
                        ...
                        for(int j = 0; j < ed_f; j++) {
                            for(int dim = 0; dim < EMB_DIM; dim++) {
                            GCN_edge_embedding_weight_float[idx + j][dim] = edge_embedding_weight_raw[l][(idx2 + j) * EMB_DIM + dim];
                            }
                        }
                        idx += ed_f;
                        idx2 += ed_f;
        ...
        case 1 : { 
                ...
                fread(GIN_node_mlp_2_weight_float, sizeof(float), NUM_LAYERS * EMB_DIM * GIN_MLP_1_OUT, f);
                fclose(f);
                ...
                fread(graph_pred_PNA_graph_mlp_3_weight_float, sizeof(float), NUM_TASK * EMB_DIM, f);
                fclose(f);
        ...
        case 2 : {
                ...
                fread(bn_weight_PNA_graph_mlp_1_weight_float_PNA, sizeof(float), 512, f);
                ...
                fread(bn_mean_PNA_graph_mlp_2_weight_float_PNA, sizeof(float), 128, f);
                ...
                fread(graph_pred_PNA_graph_mlp_3_weight_float, sizeof(float), 8, f);
    ...
    for(int l = 0; l < max_NUM_LAYERS; l++)
    {
        switch(GNN_instruction)
        {
            ...
            case 1 : GIN_node_mlp_eps_PNA_avg_deg_fixed[l] = (WT_TYPE)GIN_node_mlp_eps_PNA_avg_deg_float[l];
                     break;
        ...
        {
            GCN_convs_GIN_node_mlp_1_PNA_node_conv_bias_fixed[(l * GIN_MLP_1_OUT) + dim_out] = (WT_TYPE)GCN_convs_GIN_node_mlp_1_PNA_node_conv_bias_float[l][dim_out];

            for(int dim_in = 0; dim_in < max_dim_in; dim_in++)
            {
                switch(GNN_instruction)
                {
                    case 0 : GCN_convs_GIN_node_mlp_1_weight_fixed[(l * GIN_MLP_1_OUT * EMB_DIM) + (dim_out * EMB_DIM) + dim_in] = (WT_TYPE)GCN_convs_GIN_node_mlp_1_weight_float[l][dim_out][dim_in];
                             break;
                    
                    case 1 : GCN_convs_GIN_node_mlp_1_weight_fixed[(l * GIN_MLP_1_OUT * EMB_DIM) + (dim_out * EMB_DIM) + dim_in] = (WT_TYPE)GCN_convs_GIN_node_mlp_1_weight_float[l][dim_out][dim_in];
                             break;
                    
                    case 2 : for(int scaler = 0; scaler < NUM_SCALERS; scaler++)
                            {
                                for(int aggr = 0; aggr < NUM_AGGRS; aggr++)
                                {
                                    PNA_node_conv_weight_fixed[(l * PNA_EMB_DIM * NUM_SCALERS * NUM_AGGRS * PNA_EMB_DIM) + (dim_out * NUM_SCALERS * NUM_AGGRS * PNA_EMB_DIM) + (scaler * NUM_AGGRS * PNA_EMB_DIM) + (aggr * PNA_EMB_DIM) + dim_in] = (WT_TYPE)PNA_node_conv_weight_float[l][dim_out][scaler][aggr][dim_in];
                                }                          
                            }
                            break;
        ...
        switch(GNN_instruction)
        {
            case 0 : for(int dim_out = 0; dim_out < EMB_DIM; dim_out++)
                    {
                        GCN_convs_root_emb_weight_GIN_node_mlp_2_bias_fixed[(l * EMB_DIM) + dim_out] = (WT_TYPE)GCN_convs_root_emb_weight_GIN_node_mlp_2_bias_float[l][dim_out];
                    }
                    break;
            
            case 1 : for(int dim_out = 0; dim_out < EMB_DIM; dim_out++)
                    {
                        GCN_convs_root_emb_weight_GIN_node_mlp_2_bias_fixed[(l * EMB_DIM) + dim_out] = (WT_TYPE)GCN_convs_root_emb_weight_GIN_node_mlp_2_bias_float[l][dim_out];
                        for(int dim_in = 0; dim_in < GIN_MLP_1_OUT; dim_in++)
                        {
                            GIN_node_mlp_2_weight_fixed[(l * EMB_DIM * GIN_MLP_1_OUT) + (dim_out * GIN_MLP_1_OUT) + dim_in] = (WT_TYPE)GIN_node_mlp_2_weight_float[l][dim_out][dim_in];
                        }
                    }
                    break;
    ...
    //to load the node_embedding_weights
    ...
    switch(GNN_instruction)
    {
        case 0 : for(int i = 0; i < NUM_LAYERS * ED_FEATURE_PER_LAYER; i++)
                {
                    for(int j = 0; j < EMB_DIM; j++)
                    {
                        edge_embedding_weight_fixed[(i * EMB_DIM) + j] = (WT_TYPE)GCN_edge_embedding_weight_float[i][j];
                    }
                }
                break;

        case 1 : for(int l = 0; l < NUM_LAYERS; l++)
                {
                    for(int i = 0; i < ED_FEATURE_PER_LAYER; i++)
                    {
                        for(int dim = 0; dim < EMB_DIM; dim++)
                        {
                            edge_embedding_weight_fixed[(l * ED_FEATURE_PER_LAYER * EMB_DIM) + (i * EMB_DIM) + dim] = (WT_TYPE)GIN_edge_embedding_weight_float[l][i][dim];
                        }
                    }
                }
                break;
    ...
    int bn_max_dim_out_1;
    int bn_max_dim_out_2;
    switch(GNN_instruction)
    {
        case 0 :  for(int i = 0; i < NUM_LAYERS; i++)
                    {
                        for(int j = 0; j < EMB_DIM; j++)
                        {
                            bn_weight_PNA_graph_mlp_1_weight_fixed[(i * EMB_DIM) + j] = bn_weight_PNA_graph_mlp_1_weight_float[i][j];
                            bn_bias_PNA_graph_mlp_1_bias_fixed[(i * EMB_DIM) + j] = bn_bias_PNA_graph_mlp_1_bias_float[i][j];
                            bn_mean_PNA_graph_mlp_2_weight_fixed[(i * EMB_DIM) + j] = bn_mean_PNA_graph_mlp_2_weight_float[i][j];
                            bn_sqrt_var_PNA_graph_mlp_2_bias_fixed[(i * EMB_DIM) + j] = bn_sqrt_var_PNA_graph_mlp_2_bias_float[i][j];
                        }
                    }
                    break;
        ...
        case 2 :  for(int i = 0; i < PNA_GRAPH_MLP_1_OUT; i++)
                    {
                        bn_bias_PNA_graph_mlp_1_bias_fixed[i] = bn_bias_PNA_graph_mlp_1_bias_float[0][i];
                        if(i < PNA_GRAPH_MLP_2_OUT)
                        {
                            bn_sqrt_var_PNA_graph_mlp_2_bias_fixed[i] = bn_sqrt_var_PNA_graph_mlp_2_bias_float[0][i];
                        }
                        for(int j = 0; j < PNA_EMB_DIM; j++)
                        {
                            bn_weight_PNA_graph_mlp_1_weight_fixed[(i * EMB_DIM) + j] = bn_weight_PNA_graph_mlp_1_weight_float_PNA[i][j];
                            if(i < PNA_GRAPH_MLP_2_OUT && j < PNA_GRAPH_MLP_1_OUT)
                            {
                                bn_mean_PNA_graph_mlp_2_weight_fixed[(i * EMB_DIM) + j] = bn_mean_PNA_graph_mlp_2_weight_float_PNA[i][j];
                            }
                        }
                    }
                    break;
    ...
    {  
        graph_pred_PNA_graph_mlp_3_bias_fixed[t] = graph_pred_PNA_graph_mlp_3_bias_float[t];
        for(int dim_in = 0; dim_in < max_EMB_DIM; dim_in++)
        {
            graph_pred_PNA_graph_mlp_3_weight_fixed[(t * EMB_DIM + dim_in)] = graph_pred_PNA_graph_mlp_3_weight_float[t][dim_in];
        }
    }
    ...
    switch(GNN_instruction)
    {
        ...
        case 2 : GIN_node_mlp_eps_PNA_avg_deg_fixed[0] = 6.885701656341553;
