...
aligned_vector<WT_TYPE> PNA_node_conv_weight_fixed(DGN_PNA_NUM_LAYERS * EMB_DIM * NUM_SCALERS * NUM_AGGRS * EMB_DIM);
...
int main(int argc, char **argv) {
    ...
    OCL_CHECK(err, cl::Buffer PNA_node_conv_weight_buf(
        context,
        CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
        DGN_PNA_NUM_LAYERS * EMB_DIM * NUM_SCALERS * NUM_AGGRS * EMB_DIM * sizeof(WT_TYPE),
    ...
    krnl_GNN_compute_graphs.setArg(idx++, bn_bias_PNA_graph_DGN_MLP_1_bias_buf);
    krnl_GNN_compute_graphs.setArg(idx++, bn_mean_PNA_graph_DGN_MLP_2_weight_buf);
