...
#pragma HLS STREAM variable=embeddings depth=(4 * ceildiv(EMB_DIM, APPLY_PARALLEL))
    ...
    check_node_embedding(embeddings, message, node_feature_in, node_embedding_h_atom_embedding_list_weight_in, layer_num, num_of_nodes);
...
#pragma HLS INLINE off
    ...
    if (i == 0)
        load_input_node_embeddings(embeddings, node_feature_in, node_embedding_h_atom_embedding_list_weight_in, message, num_of_nodes);
    else
