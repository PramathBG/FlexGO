...
#pragma HLS LOOP_TRIPCOUNT min=0 max=ANALYSIS_MAX_EDGES avg=ceildiv(ANALYSIS_AVG_EDGES, EDGE_PARALLEL)
        ...
        WT_TYPE GCN_norm = GCN_norms[pe_id][e];
        if(instruction == GIN)
            GCN_norm = (FM_TYPE)1;
...
#pragma HLS UNROLL
                ...
                if(dim < EMB_DIM)
                {
                    FM_TYPE message_tmp [NUM_AGGRS];

                    for(int aggr = 0; aggr < NUM_AGGRS; aggr++)
                    {
                        message_tmp[aggr] = message[v][dim][aggr];
                    }
...
#pragma HLS UNROLL
                        ...
                        message_tmp[0] += GCN_norm * ap_fixed_relu(total_embed);
                    }
                    else if(instruction == PNA)
                    {
                        FM_TYPE embedding_dim = node_embedding[dim_offset];
                        message_tmp[AGGR_MEAN] += embedding_dim;
                        message_tmp[AGGR_STD] += FM_TYPE(embedding_dim * embedding_dim);
                        if (embedding_dim < message_tmp[AGGR_MIN])
                            message_tmp[AGGR_MIN] = embedding_dim;
                        if (embedding_dim > message_tmp[AGGR_MAX])
                            message_tmp[AGGR_MAX] = embedding_dim;
                    }
                    for(int aggr = 0; aggr < NUM_AGGRS; aggr++)
                    {
                        message[v][dim][aggr] = message_tmp[aggr];
                    }
                }
            }
        }
