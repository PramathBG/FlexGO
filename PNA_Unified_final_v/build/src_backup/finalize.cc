...
void global_mean_pooling(
    ...
    FM_TYPE PNA_h_graph[PNA_EMB_DIM],
    int num_of_nodes
);
void check_linear(
    hls::stream<ne_out_t>& GCN_h_graph,
...
#pragma HLS UNROLL
                    ...
                    if (dim < EMB_DIM)
                    {
                        ...
                        bn_sqrt_var_dim = bn_sqrt_var_final[dim];
                        bn_weight_dim = bn_weight_final[dim];
                        bn_mean_dim = bn_mean_final[dim];
                        bn_bias_dim = bn_bias_final[dim];
                    }
                    for(int nd_offset = 0; nd_offset < NODE_PARALLEL; nd_offset++)
                    {
                        ...
                        if(nd < num_of_nodes)
                        {
                            FM_TYPE activation = message[nd % EDGE_PARALLEL][nd / EDGE_PARALLEL][dim][0];
...
#pragma HLS UNROLL
    ...
    else if(instruction == GIN || instruction == PNA)
    {
...
#pragma HLS UNROLL
                ...
                PNA_h_graph[dim] = h_graph_el / num_of_nodes;
            }
        }
    }
}
...
#pragma HLS DATAFLOW
    ...
    hls::stream<mlp_xfer_t> PNA_graph_mlp_1_out ("PNA_graph_mlp_1_out");
#pragma HLS STREAM variable=PNA_graph_mlp_1_out depth=ceildiv(PNA_GRAPH_MLP_1_OUT, APPLY_PARALLEL)

    FM_TYPE PNA_graph_mlp_2_out[PNA_GRAPH_MLP_2_OUT];

    linear_output_stationary<PNA_EMB_DIM, PNA_GRAPH_MLP_1_OUT, APPLY_PARALLEL, true>(
        PNA_h_graph,
        PNA_graph_mlp_1_weights,
        PNA_graph_mlp_1_bias,
        PNA_graph_mlp_1_out
    ...
    linear_input_stationary<PNA_GRAPH_MLP_1_OUT, PNA_GRAPH_MLP_2_OUT, APPLY_PARALLEL, true>(
        PNA_graph_mlp_1_out,
        PNA_graph_mlp_2_weights,
        PNA_graph_mlp_2_bias,
        PNA_graph_mlp_2_out
    ...
    linear<PNA_GRAPH_MLP_2_OUT, NUM_TASK, NUM_TASK, false>(
        PNA_graph_mlp_2_out,
        PNA_graph_mlp_3_weights,
        PNA_graph_mlp_3_bias,
        result
