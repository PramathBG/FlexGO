...
#pragma HLS INTERFACE m_axi depth=(1) port=GIN_node_mlp_2_weights_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=(1) port=GCN_convs_root_emb_weight_GIN_node_mlp_2_bias_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=(1) port=PNA_node_conv_weight_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=(1) port=bn_weight_PNA_graph_mlp_1_weight_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=(1) port=bn_bias_PNA_graph_mlp_1_bias_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=(1) port=bn_mean_PNA_graph_mlp_2_weight_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=(1) port=bn_sqrt_var_PNA_graph_mlp_2_bias_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=(1) port=graph_pred_PNA_graph_mlp_3_weight_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=(1) port=graph_pred_PNA_graph_mlp_3_bias_in offset=slave bundle=mem
#pragma HLS INTERFACE m_axi depth=(1) port=avg_deg_in offset=slave bundle=mem

    instruction = instruction_in;
    max_EMB_DIM = (instruction == PNA) ? PNA_EMB_DIM : EMB_DIM;
    max_iter = ceildiv(max_EMB_DIM, SCATTER_PARALLEL);
    max_NUM_LAYERS = (instruction == PNA) ? PNA_NUM_LAYERS : NUM_LAYERS;
...
#pragma HLS LOOP_TRIPCOUNT min=ANALYSIS_NUM_GRAPHS max=ANALYSIS_NUM_GRAPHS avg=ANALYSIS_NUM_GRAPHS
        ...
        if(reload_weights_graph)
        {
            ...
            load_weights(
                ...
                GIN_node_mlp_2_weights_in[weights_ndx],
                GCN_convs_root_emb_weight_GIN_node_mlp_2_bias_in[weights_ndx],
                PNA_node_conv_weight_in[weights_ndx],
                edge_embedding_weight_in[weights_ndx],
                bn_weight_PNA_graph_mlp_1_weight_in[weights_ndx],
                bn_bias_PNA_graph_mlp_1_bias_in[weights_ndx],
                bn_mean_PNA_graph_mlp_2_weight_in[weights_ndx],
                bn_sqrt_var_PNA_graph_mlp_2_bias_in[weights_ndx],
                graph_pred_PNA_graph_mlp_3_weight_in[weights_ndx],
                graph_pred_PNA_graph_mlp_3_bias_in[weights_ndx],
...
#pragma HLS STREAM variable=GCN_finalize_embeddings depth=(4 * (ceildiv(EMB_DIM, APPLY_PARALLEL)))
    ...
    if(NUM_LAYERS % 2 == 0)
        finalize(GCN_finalize_embeddings, 
                messages_pong, 
                PNA_graph_mlp_1_weights,
                PNA_graph_mlp_1_bias,
                PNA_graph_mlp_2_weights,
                PNA_graph_mlp_2_bias,
                PNA_graph_mlp_3_weights,
                PNA_graph_mlp_3_bias, 
                graph_pred_PNA_graph_mlp_3_weights, 
                graph_pred_PNA_graph_mlp_3_bias, 
                result, 
                num_of_nodes);
    else 
        finalize(GCN_finalize_embeddings, 
