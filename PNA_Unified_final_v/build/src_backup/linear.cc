...
#pragma HLS INLINE off
    ...
    constexpr int MIN_PARALLEL_DIM_OUT = (PARALLEL < DIM_OUT) ? PARALLEL : DIM_OUT;

#pragma HLS ARRAY_PARTITION variable=weight complete dim=1
#pragma HLS ARRAY_PARTITION variable=weight cyclic factor=MIN_PARALLEL_DIM_OUT dim=2
...
//template instantiation for PNA
template void linear_output_stationary<PNA_EMB_DIM, PNA_GRAPH_MLP_1_OUT, APPLY_PARALLEL, true>(
    FM_TYPE input[PNA_EMB_DIM],
    WT_TYPE weight[PNA_GRAPH_MLP_1_OUT][PNA_EMB_DIM],
    WT_TYPE bias[PNA_GRAPH_MLP_1_OUT],
    hls::stream<array<FM_TYPE, APPLY_PARALLEL>>& output
);

template void linear_input_stationary<PNA_GRAPH_MLP_1_OUT, PNA_GRAPH_MLP_2_OUT, APPLY_PARALLEL, true>(
    hls::stream<array<FM_TYPE, APPLY_PARALLEL>>& input,
    WT_TYPE weight[PNA_GRAPH_MLP_2_OUT][PNA_GRAPH_MLP_1_OUT],
    WT_TYPE bias[PNA_GRAPH_MLP_2_OUT],
    FM_TYPE output[PNA_GRAPH_MLP_2_OUT]
);

template void linear<PNA_GRAPH_MLP_2_OUT, NUM_TASK, NUM_TASK, false>(
    FM_TYPE input[PNA_GRAPH_MLP_2_OUT],
    WT_TYPE weight[NUM_TASK][PNA_GRAPH_MLP_2_OUT],
    WT_TYPE bias[NUM_TASK],
    FM_TYPE output[NUM_TASK]
);
